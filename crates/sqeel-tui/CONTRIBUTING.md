# Contributing to `sqeel-tui`

Thanks for considering a contribution. `sqeel-tui` is pre-1.0 and the public
...
```bash
git clone git@github.com:kryptic-sh/sqeel-tui.git
cd sqeel-tui
...
Patch bumps follow the BCTP flow (Bump → Commit → Tag → Push). This crate
depends on `sqeel-core` from crates.io; bump that crate first when changes
require it.
