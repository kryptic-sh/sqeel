...
// Row uses usize so files with more than u16::MAX lines scroll correctly.
// Col, width, and height remain u16 — they are screen dimensions.
#[derive(Default, Debug)]
pub struct Viewport {
    row: AtomicUsize,
    rest: AtomicU64, // bits 0-15: col, bits 16-31: width, bits 32-47: height
}
...
impl Clone for Viewport {
    fn clone(&self) -> Self {
        Viewport {
            ...
            rest: AtomicU64::new(self.rest.load(Ordering::Relaxed)),
        }
    }
}

impl Viewport {
    pub fn scroll_top(&self) -> (usize, u16) {
        let row = self.row.load(Ordering::Relaxed);
        let col = self.rest.load(Ordering::Relaxed) as u16;
        (row, col)
    }

    pub fn rect(&self) -> (usize, u16, u16, u16) {
        let row = self.row.load(Ordering::Relaxed);
        let rest = self.rest.load(Ordering::Relaxed);
        let col = rest as u16;
        let width = (rest >> 16) as u16;
        let height = (rest >> 32) as u16;
        (row, col, width, height)
    }

    pub fn position(&self) -> (usize, usize, usize, usize) {
        let (row_top, col_top, width, height) = self.rect();
        let col_top = col_top as usize;
    ...
    fn store(&self, row: usize, col: u16, width: u16, height: u16) {
        self.row.store(row, Ordering::Relaxed);
        let rest = (col as u64) | ((width as u64) << 16) | ((height as u64) << 32);
        self.rest.store(rest, Ordering::Relaxed);
    }
    ...
    pub fn scroll(&mut self, rows: isize, cols: i16) {
        ...
        let col = (*rest as i64 + cols as i64).max(0) as u16;
        *rest = (*rest & 0xffff_ffff_ffff_0000) | (col as u64);
    }
}
...
impl<'a> TextArea<'a> {
    ...
    fn scroll_top_col(&self, prev_top: u16, width: u16) -> u16 {
        ...
        next_scroll_top(prev_top as usize, cursor as usize, width as usize) as u16
    }
}
...
impl Widget for &TextArea<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let (text, style) = if !self.placeholder.is_empty() && self.is_empty() {
            ...
            (self.text_widget(top_row, height as _), self.style())
        };
